...
fun Editor(ipAddress: String, editorViewModel: EditorViewModel = viewModel()) {
    ...
    val languageServerSocket by remember { mutableStateOf(LanguageServerSocket(ipAddress))}
    var editFieldText = currentFile
    editorViewModel.address = ipAddress
    webSocketScope.launch {
        if(!sessionStarted) {
            sessionStarted = true;
            languageServerSocket.startSession(this, messageFlow)
//            val session = startSession(ipAddress)
//            editorViewModel.outgoing = session.outgoing
//            //var launcher = LSPLauncher.createClientLauncher(languageServerSocket, session.incoming.receiveAsFlow().buffer(), session.outgoing.)
        ...
        topBar = {
            TopAppBar(title = { Row( verticalAlignment = Alignment.CenterVertically,modifier = Modifier.fillMaxWidth()) {
                ...
                Button(onClick = {
                    drawerState.open()
                    listenerScope.launch {
                        languageServerSocket.socketSession.outgoing.send(Frame.Text(testInit))
